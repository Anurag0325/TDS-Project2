...
def count_wednesdays():
    ...
    count = sum(1 for d in range((end_date - start_date).days + 1)
                if (start_date + datetime.timedelta(days=d)).weekday() == 2)  # Wednesday is weekday 2
...
def sum_data_values_from_html(html_content: str) -> int:
    ...
    try:
        ...
        total = sum(int(div.get("data-value", 0)) for div in soup.find_all("div", class_="foo"))
        return total
    except Exception as e:
        return f"Error: {str(e)}"
...
def convert_webp_to_png_opencv(webp_file: str, output_dir: str = "converted_images"):
    ...
    try:
        ...
        if image is None:
            raise ValueError("❌ OpenCV could not read the .webp file. File may be corrupted.")

        # Generate output file name
        png_file = os.path.join(output_dir, os.path.splitext(os.path.basename(webp_file))[0] + ".png")
...
def extract_zip_file(zip_path: str) -> dict:
    ...
    """
    extract_folder = os.path.join(EXTRACTED_DIR, os.path.splitext(os.path.basename(zip_path))[0])
    os.makedirs(extract_folder, exist_ok=True)
...
def process_question_2(extract_folder: str):
    ...
    for file_name, encoding in file_encodings.items():
        ...
        if os.path.exists(file_path):
            df = pd.read_csv(file_path, encoding=encoding, delimiter="\t" if file_name.endswith(".txt") else ",")
    ...
    try:
        ...
        for filename in sorted(os.listdir(folder_path)):  # Process in order
            if filename.endswith(".txt"):
                ...
                # Replace "IITM" (case-insensitive) with "IIT Madras"
                modified_content = [line.replace("IITM", "IIT Madras").replace("iitm", "IIT Madras").replace("IItM", "IIT Madras") for line in content]
                
                # Write back with original line endings
                with open(file_path, "w", encoding="utf-8", newline="") as file:
                    file.writelines(modified_content)

                all_content.extend(modified_content)  # Append to hash calculation

        # ✅ Compute SHA-256 hash after modifications
        sha256_hash = hashlib.sha256("".join(all_content).encode("utf-8")).hexdigest()
...
def calculate_filtered_size(folder_path: str):
    ...
    """
    total_size = 0
    CUTOFF_DATETIME = datetime.datetime(2008, 3, 19, 5, 54)  # 19 Mar 2008, 5:54 AM IST
        ...
        for filename in os.listdir(folder_path):
            ...
            if os.path.isfile(file_path):  # Ensure it's a file
                ...
                if file_size >= SIZE_THRESHOLD and modified_time >= CUTOFF_DATETIME:
                    ...
                    print(f"✔ {filename}: {file_size} bytes, Modified: {modified_time}")

        print(f"📏 Total Size of Matching Files: {total_size} bytes")
        return str(total_size)  # 🔥 Return total size as a string (not a dictionary)
...
def process_zip_and_hash(extract_path):
    ...
    renamed_files = {}
    for filename in sorted(os.listdir(target_folder), key=lambda x: x.encode("utf-8")):  # LC_ALL=C sorting
    ...
    all_contents = []
    for filename in sorted(os.listdir(target_folder), key=lambda x: x.encode("utf-8")):  # Correct sorting
        file_path = os.path.join(target_folder, filename)
        if os.path.isfile(file_path):
            with open(file_path, "r", encoding="utf-8", errors="ignore") as f:
                lines = f.readlines()
                for line in lines:
                    all_contents.append(f"{filename}:{line.rstrip()}")  # Strip trailing spaces & newlines

    # Correct sorting for `LC_ALL=C`
    sorted_text = "\n".join(sorted(all_contents, key=lambda x: x.encode("utf-8")))  # Sort using byte order
...
def count_tokens(text: str):
    ...
    encoding = tiktoken.encoding_for_model("gpt-4o")  # Use "gpt-4o" or another model
...
def get_weather_forecast(text: str):
    ...
    datelist = pd.date_range(datetime.today(), periods=len(daily_summary_list)).strftime('%Y-%m-%d').tolist()
...
def extract_city_country_latitude(question: str):
    ...
    if match:
        ...
        lat_type = "max" if "maximum latitude" in question.lower() else "min" if "minimum latitude" in question.lower() else None
...
def get_latitude(city: str, country: str, lat_type: str):
    ...
    latitude = float(bounding_box[1]) if lat_type == "max" else float(bounding_box[0])
...
def extract_github_query_params(question):
    ...
    city_match = re.search(r"located in the city ([\w\s]+?)(?:\s+with|\s+followers)", question, re.IGNORECASE)
    
    # Extracts minimum followers count
    followers_match = re.search(r"over (\d+) followers", question, re.IGNORECASE)
    
    # Extracts cutoff datetime
    cutoff_match = re.search(r"after\s+(\d{1,2}/\d{1,2}/\d{4},\s+\d{1,2}:\d{1,2}:\d{2}\s*[APMapm]*)", question)

    city = city_match.group(1).strip() if city_match else "Chennai"  # Default: Chennai
    min_followers = int(followers_match.group(1)) if followers_match else 110  # Default: 110 followers
    cutoff_datetime = None
    
    if cutoff_match:
        try:
            cutoff_datetime = datetime.strptime(cutoff_match.group(1), "%m/%d/%Y, %I:%M:%S %p").replace(tzinfo=timezone.utc)
        except ValueError:
...
def get_newest_github_user(question):
    ...
    city, min_followers, cutoff_datetime = extract_github_query_params(question)
    ...
    response = requests.get(f"{GITHUB_API_URL}?q={query}&sort=joined&order=desc")
...
def clean_and_process_data(data):
    ...
    df.columns = ["Group", "Maths", "Physics", "English", "Economics", "Biology"][:len(df.columns)]
...
def extract_conditions_from_question(question):
    ...
    subject_match = re.search(r'sum total of ([A-Za-z]+) marks', question, re.IGNORECASE)
    marks_condition_match = re.search(r'scored (\d+) or (more|less)', question, re.IGNORECASE)
    condition_subject_match = re.search(r'in ([A-Za-z]+)', question, re.IGNORECASE)
    group_range_match = re.search(r'groups (\d+)[^\d]+(\d+)', question, re.IGNORECASE)
    
    subject = subject_match.group(1) if subject_match else "English"
    required_marks = int(marks_condition_match.group(1)) if marks_condition_match else 69
    comparison = marks_condition_match.group(2) if marks_condition_match else "more"
    condition_subject = condition_subject_match.group(1) if condition_subject_match else "Maths"
...
def calculate_total_marks(pdf_path, question):
    ...
    subject, required_marks, comparison, condition_subject, start_page, end_page = extract_conditions_from_question(question)
    raw_data = extract_tables_with_pdfplumber(pdf_path, start_page, end_page)
    df = clean_and_process_data(raw_data)
    
    # Apply filters correctly
    if comparison == "more":
        filtered_df = df[(df["Group"].between(start_page, end_page)) & (df[condition_subject] >= required_marks)]
    else:
        filtered_df = df[(df["Group"].between(start_page, end_page)) & (df[condition_subject] < required_marks)]
...
async def answer_question(
    ...
    try:
        ...
        if "analyze the sentiment" in question.lower():
            ...
            sentiment = ask_llm(f"Analyze the sentiment of the following text. Respond ONLY with 'GOOD', 'BAD', or 'NEUTRAL':\n\n{text}")
        ...
        if file is not None:  # Check if a file was uploaded
            ...
            if file.filename.endswith(".zip"):
                extract_path = os.path.join(EXTRACTED_DIR, os.path.splitext(file.filename)[0])
                ...
                try:
                    with zipfile.ZipFile(file_path, "r") as zip_ref:
                        ...
                        # }
                        print(f"📂 Extracted Files: {extracted_files}")  # Debug Log
